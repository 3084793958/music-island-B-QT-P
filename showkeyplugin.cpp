    ...
    : QObject(parent)
{
    ...
    {
        if (m_popupWidget->now_playing < 0 || m_popupWidget->now_playing >= m_pluginWidget->play_files.size())
        {
            m_popupWidget->now_playing = 0;
            if (m_pluginWidget->play_files.size() == 0)
            {
                return;
            }
        }
...
void ShowKeyPlugin::pluginStateSwitched()
{
    ...
    if (disabledNew) disable_plugin();
    if (m_proxyInter) m_proxyInter->saveValue(this, "disabled", disabledNew);
    if (disabledNew)
    {
        if (m_proxyInter) m_proxyInter->itemRemoved(this, pluginName());
    }
    else
    {
        if (m_proxyInter) m_proxyInter->itemAdded(this, pluginName());
...
void ShowKeyPlugin::timer_update()
{
    ...
    if (m_pluginWidget->can_clone)
    {
        ...
        if(play_main->position()==0&&!m_pluginWidget->already_start && m_pluginWidget->play_files.size() != 0)
        {
            if (m_popupWidget->now_playing < 0 || m_popupWidget->now_playing >= m_pluginWidget->play_files.size())
            {
                m_popupWidget->now_playing = 0;
...
void ShowKeyPlugin::color_setting()
{
...
void ShowKeyPlugin::font_setting()
{
    bool ok;
    QFont now_font=m_pluginWidget->font();
...
void ShowKeyPlugin::disable_plugin()
{
    play_main->pause();
    if (!m_pluginWidget || !m_popupWidget) return;
